...
def conditional_borrowing_book():
    ...
    result = db_manage.sql_conditional_borrowing_book(book_id, book_name, book_publisher, borrow_time_satrt,
                                                      borrow_time_end, user_name)
    if not result[0]:
...
def conditional_borrow_record():
    ...
    if borrow_time_satrt and borrow_time_end and borrow_time_satrt > borrow_time_end:
        return jsonify({"status": -1, "message": "起始时间不能大于结束时间", "data": []})
    result = db_manage.sql_conditional_borrow_record(book_id, book_name, book_publisher, borrow_time_satrt,
                                                     borrow_time_end, user_name)
    if not result[0]:
        return jsonify({"status": -1, "message": "fail", "data": []})
    return jsonify({"status": 0, "message": "success", "data": result[1]})
...
def borrowing_condition():
    data_dict = {}
    data_dict["user_number"], data_dict["book_number"], data_dict["borrowing_number"], data_dict[
        "record_number"] = db_manage.sql_borrowing_condition1()
