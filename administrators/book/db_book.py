...
def sql_borrow_book(user_id, book_id):
    time = set_time()
    sql = "insert into borrow_info (user_id,book_id,borrow_time,return_time,state) values ({},{},'{}','{}','{}')".format(
        user_id, book_id, time.today(), time.next_month(), 1)
...
def sql_return_book(book_id, user_id):
    time = set_time()
    sql = "UPDATE borrow_info set state = '0',actual_return_time = '{}' where book_id = {} and user_id = {} ".format(
        time.today(), book_id, user_id)
